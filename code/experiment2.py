...
mu_1, mu_2, ..., mu_{K-M} = np.repeat(theta - eps, K - M)
mu_{K-M+1}, ..., mu_{K} = np.repeat(theta + eps, M)
"""
...
def main():
    ...
    for K in Ks:
        ...
        for delta in [1e-16]:
            fp_pickle = f"{SAVEDIR}/result_exp2_[K={K},delta={delta}].pkl"

            args_list = []
            for m in [int(0.3*K), int(0.5*K), int(0.7*K)]:
                for L in [int(0.2*K), int(0.4*K), int(0.6*K), int(0.8*K)]:
                    mus = np.repeat(theta - eps, K - m)
                    mus = np.append(mus, np.repeat(theta + eps, m))
                    mus = tuple(mus)
